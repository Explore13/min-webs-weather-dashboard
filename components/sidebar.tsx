...
import { Trash2, Edit, Palette, Loader2, Database, Thermometer, MapPin } from 'lucide-react'
import { ColorRuleEditor } from "./color-rule-editor"

export function Sidebar() {
  const { polygons, deletePolygon, updatePolygon, isLoadingWeather } = useStore()
                        ...
                        {editingPolygon === polygon.id ? (
                          ...
                          <>
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <CardTitle className="text-sm text-blue-800">{polygon.name}</CardTitle>
                            </div>
                            <div className="flex gap-1">
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleNameEdit(polygon.id, polygon.name)}
